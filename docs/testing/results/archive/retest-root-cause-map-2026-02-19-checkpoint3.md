...
| Incomplete-run interpretation risk | Usage cap interruption is treated as scenario failure in some rows without explicit â€œincompleteâ€ guardrail. | Medium | Add explicit â€œINCOMPLETE (usage/runtime)â€ status class in matrix template and exclude from fail counts. |
...
- Several findings were weak because instrumentation proved â€œsomething happenedâ€ but not â€œthe intended action happened.â€
