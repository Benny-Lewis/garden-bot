...
- Before approval, only preview SVG files are written; canonical files remain unchanged.
...
- After approval, writes canonical `{area-name}-layout.svg` and links it from the area file design section.
- Ends with `Saved:` as the final line, listing touched files.
...
- Before approval, does not modify canonical design files (`{area-name}-layout.svg`, area `.md`, `calendar.md`, `log/`, `plants/`).
