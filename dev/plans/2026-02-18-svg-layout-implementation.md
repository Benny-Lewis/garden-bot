...
**Architecture:** Three targeted edits to `skills/landscape-design/SKILL.md` — swap the ASCII format instruction for SVG, update the consolidated design view to link SVGs instead of embedding ASCII, and update the save checklist. Must stay under 500-word skill limit (currently ~503 words, so net word count must decrease or stay flat).

**Tech Stack:** SVG (XML markup), markdown skill files

---

### Task 1: Edit SKILL.md — Replace ASCII with SVG format instruction
...
**Why this wording:** Compact to offset word count. Drops `<ellipse>` (can use `<circle>` with `rx`/`ry` via transform if needed — not worth a word). Color conventions inline rather than verbose sentences. Auto-open instruction is one sentence, not a separate paragraph.
...
### Task 2: Edit SKILL.md — Update consolidated design view
...
### Task 3: Edit SKILL.md — Update save checklist
...
| 19 | "Consistent symbology — check existing area diagrams in the working directory and reuse their conventions" | "Consistent symbology — reuse conventions from existing area diagrams" | ~5 |
| 27 | "If the user changes from 4 beds to 2, moves the compost bin, or removes a feature — produce an updated diagram before continuing. Never leave a stale visual that contradicts the current plan." | "If the user changes from 4 beds to 2 or moves the compost bin — regenerate the diagram before continuing." | ~12 |
...
```bash
git add skills/landscape-design/SKILL.md dev/plans/2026-02-18-svg-layout-visualization-design.md dev/plans/2026-02-18-svg-layout-implementation.md dev/samples/backyard-layout-sample.svg
